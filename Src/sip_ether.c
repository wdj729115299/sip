...
static __u8 input(struct skbuff *pskb, struct net_device *dev)
{
		...
		|| samemac(skb->phy.ethh->h_dest, dev->hwbroadcast))/*广播数据?*/
	{
		...
		DBGPRINT(DBG_LEVEL_NOTES,"eth proto [%d]\n", htons(skb->phy.ethh->h_proto));
...
EXITinput:
	DBGPRINT(DBG_LEVEL_TRACE,"error <==input\n");
	return 0;
}
...
#if 0
	dev->ip_host.s_addr = inet_addr("192.168.1.100");
...
#endif
#if 1	
	dev->ip_host.s_addr = inet_addr("10.0.2.15");
	dev->ip_gw.s_addr = inet_addr("10.0.2.1");
	dev->ip_netmask.s_addr = inet_addr("255.255.255.0");
	dev->ip_broadcast.s_addr = inet_addr("10.0.2.255");
#else
